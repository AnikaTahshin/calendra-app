...
export default async function EventsPage() {
  ...
  return (
   <section className="flex flex-col items-center gap-16 animate-fade-in">
        ...
        hover:border-blue-500 rounded-2xl shadow-accent-foreground text-2xl font-black" asChild>
        ...
        {/* SHOW ALL THE ACTIVE/ INACTIVE EVENTS */}
        {events.length > 0 ? (
              <div className="grid gap-4 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 p-10">
                {events.map(event => (
                  <EventCard key={event.id} {...event} description={event.description ?? undefined} />
                ))}
              </div>
            ) : (
